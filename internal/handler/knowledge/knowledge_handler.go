...
func NewKnowledgeHandler(service knowledgeservice.KnowledgeService) *KnowledgeHandler {
...
func (h *KnowledgeHandler) Fields(c *gin.Context) {
	result, err := h.service.SearchFields(strings.TrimSpace(c.Query("q")), limit(c))
	if err != nil {
		utils.ErrorJSONResponseWithMessage(c, http.StatusInternalServerError, err.Error())
		return
	}
	utils.JSONResponse(c, http.StatusOK, "Knowledge fields retrieved successfully", result, nil)
}
func (h *KnowledgeHandler) CreateField(c *gin.Context) {
	var req dto.CreateReferenceRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		utils.ErrorJSONResponseWithMessage(c, http.StatusBadRequest, "name is required")
		return
	}
	item, err := h.service.CreateField(req.Name)
	if err != nil {
		utils.ErrorJSONResponseWithMessage(c, http.StatusBadRequest, err.Error())
		return
	}
	utils.JSONResponse(c, http.StatusCreated, "Knowledge field created successfully", item, nil)
}
