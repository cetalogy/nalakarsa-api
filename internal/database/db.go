...
func InitDB(cfg *config.Config) (*gorm.DB, error) {
	...
	latestSchemaStatements := []string{
		...
		`UPDATE knowledge_domains SET name = trim(regexp_replace(regexp_replace(name, '^[(][0-9]+[)][[:space:]]*', ''), '[[:space:]]+[0-9]+$', '')) WHERE name ~ '^[(][0-9]+[)]' OR name ~ '[[:space:]]+[0-9]+$'`,
		`UPDATE knowledge_subdomains SET name = trim(regexp_replace(regexp_replace(name, '^[(][0-9]+[)][[:space:]]*', ''), '[[:space:]]+[0-9]+$', '')) WHERE name ~ '^[(][0-9]+[)]' OR name ~ '[[:space:]]+[0-9]+$'`,
		...
		`DELETE FROM knowledge_fields a USING knowledge_fields b WHERE a.id > b.id AND a.subdomain_id = b.subdomain_id AND a.name = b.name`,
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_knowledge_field_name ON knowledge_fields (subdomain_id, name)`,
