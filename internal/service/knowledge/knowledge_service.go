...
func (s *knowledgeService) CreateField(name string) (*dto.KnowledgeFieldSuggestion, error) { name, err := reference.NormalizeName(name); if err != nil { return nil, err }; item, err := s.repo.Create(&model.KnowledgeField{Name: name, IsActive: true}); if err != nil { return nil, err }; return &dto.KnowledgeFieldSuggestion{ID: item.ID, Name: item.Name}, nil }
